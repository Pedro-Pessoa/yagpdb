...
func (t *TemporaryResponse) Send(data *Data) ([]*discordgo.Message, error) {
	...
	time.AfterFunc(t.Duration, func() {
		...
		if len(msgs) > 1 {
			...
			_ = data.Session.ChannelMessagesBulkDelete(data.Msg.ChannelID, ids)
		} else {
			_ = data.Session.ChannelMessageDelete(data.Msg.ChannelID, msgs[0].ID)
		}
	})
	return msgs, nil
}
...
func (fe *FallbackEmebd) Send(data *Data) ([]*discordgo.Message, error) {
	...
	return SplitSendMessage(data.Session, data.Msg.ChannelID, content, &discordgo.MessageAllowedMentions{})
}
...
func SplitSendMessage(s *discordgo.Session, channelID int64, contents string, allowedMentions *discordgo.MessageAllowedMentions) ([]*discordgo.Message, error) {
