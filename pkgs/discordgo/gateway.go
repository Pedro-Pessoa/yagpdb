...
func (g *GatewayConnection) identify() error {
	if g.manager.shardCount > 1 {
		g.manager.session.Identify.Shard = &[2]int{g.manager.shardID, g.manager.shardCount}
	}

	g.manager.session.Identify.Token = g.manager.session.Token
	g.manager.session.Identify.Shard = nil

	op := outgoingEvent{
		Operation: 2,
		Data:      g.manager.session.Identify,
	}
