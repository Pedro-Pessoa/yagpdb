...
var execTests = []execTest{
	...
	// Range.
	{"range string", "{{range .X}}-{{.}}-{{end}}", "-120-", tVal, true},
	{"range empty string no else", "{{range .XEmpty}}-{{.}}-{{end}}", "", tVal, true},
	{"range string else", "{{range .X}}-{{.}}-{{else}}-EMPTY-{{end}}", "-120-", tVal, true},
	{"range empty string else", "{{range .XEmpty}}-{{.}}-{{else}}-EMPTY-{{end}}", "-EMPTY-", tVal, true},
	{"range chin", "{{range $k, $v := .Chin}}-{{- $k}} {{. | printf \"%c\" -}}-{{end}}", "-0 日--3 本--6 語-", tVal, true},
	...
	{"while falsy value", "{{while .MSIEmpty}}test{{end}}", "", tVal, true},
	{"while falsy value else", "{{while .MSIEmpty}}test{{else}}falsy{{end}}", "falsy", tVal, true},
...
func TestExecuteOnNewTemplate(t *testing.T) {
	...
	_, _ = new(Template).Parse("")
	_, _ = new(Template).New("abc").Parse("")
	_ = new(Template).Execute(nil, nil)                // returns an error (but does not crash)
	_ = new(Template).ExecuteTemplate(nil, "XXX", nil) // returns an error (but does not crash)
}
...
func TestMessageForExecuteEmpty(t *testing.T) {
	...
	_, _ = tmpl.AddParseTree("secondary", tests.Tree)
	err = tmpl.Execute(&b, 0)
	if err == nil {
