...
func call(fn reflect.Value, args ...reflect.Value) (reflect.Value, error) {
	...
	out, err, _ := SafeCall(fn, argv)
	return out, err
}

// SafeCall runs fun.Call(args), and returns the resulting value and error, if
// any. If the call panics, the panic value is returned as an error.
func SafeCall(fun reflect.Value, args []reflect.Value) (val reflect.Value, err error, paniced bool) {
	defer func() {
		if r := recover(); r != nil {
			paniced = true
			if e, ok := r.(error); ok {
	...
	if len(ret) == 2 && !ret[1].IsNil() {
		return ret[0], ret[1].Interface().(error), false
	}
	return ret[0], nil, false
}
	...
	for i := 0; i < len(b); i++ {
		...
		if c < utf8.RuneSelf {
			...
			case '=':
				_, _ = w.Write(jsEq)
			default:
				_, _ = w.Write(jsLowUni)
				t, b := c>>4, c&0x0f
				_, _ = w.Write(hex[t : t+1])
				_, _ = w.Write(hex[b : b+1])
			}
		} else {
			// Unicode rune.
...
// Placeholder functions which are executed by the interpreter.
