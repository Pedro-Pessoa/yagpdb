...
var parseTests = []parseTest{
	...
	{"while 1 var", "{{while $x := .SI}}{{.}}{{end}}", noError,
		`{{while $x := .SI}}{{.}}{{end}}`},
	{"while []int with break", "{{while .SI}}{{break}}{{.}}{{end}}", noError,
		`{{while .SI}}{{break}}{{.}}{{end}}`},
	{"while []int with break in else", "{{while .SI}}{{while .SI}}{{.}}{{else}}{{break}}{{end}}{{end}}", noError,
		`{{range .SI}}{{range .SI}}{{.}}{{else}}{{break}}{{end}}{{end}}`},
	{"while []int with continue", "{{while .SI}}{{continue}}{{.}}{{end}}", noError,
		`{{while .SI}}{{continue}}{{.}}{{end}}`},
