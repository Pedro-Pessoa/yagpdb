	...
	for _, v := range commands {
		if common.ContainsInt64Slice(ms.Roles, v.RoleId) {
			if c.ModeSettings().SingleAutoToggleOff {
				_ = common.BotSession.GuildMemberRoleRemove(ms.Guild.ID, ms.ID, v.RoleId)
			} else {
...
func (c *CommonRoleSettings) MaybeScheduleRoleRemoval(ctx context.Context, ms *dstate.MemberState) error {
	...
	_, err := schEvtsModels.ScheduledEvents(qm.Where("event_name='remove_member_role' AND  guild_id = ? AND (data->>'user_id')::bigint = ? AND (data->>'role_id')::bigint = ?", ms.Guild.ID, ms.ID, c.RoleId)).DeleteAll(ctx, common.PQ)
	if err != nil {
		return err
	}
